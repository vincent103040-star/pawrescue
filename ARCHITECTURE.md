...
| **志工主鍵是 email，而出勤紀錄用「姓名字串」關聯** | 改信箱等於換人；跨系統對不起來；email 是個資卻被迫在每次呼叫裡傳。更立即的代價是姓名會漂移 —— 登入時 Google 會改寫 `volunteers.name`、代理簽到由人輸入、全形空白 —— 而 `attendance_records` 只記姓名 | 整合藍圖 R2，未實作。已用 `attendanceOwnership.ts` 止血（報名紀錄的 email 為主、姓名正規化為輔），但根治要在 `attendance_records` 加上不會變的識別碼 |
