...
interface SopItem {
  ...
  completedAt?: string;
  /**
   * The manual section and demonstration video a coordinator attached, already
   * resolved by the server. Null when nothing is attached, and also null when
   * the material was deleted after being linked -- so this never renders a
 ...
 * own: people do not watch a demonstration to learn, they watch it because
 * they are about to do the thing. So it opens over the checklist and closes
 * back onto the tick box, rather than navigating anybody away from their work.
 */
const DutyMaterialModal: React.FC<{
  title: string;
  material: NonNullable<SopItem['material']>;
  onClose: () => void;
}> = ({ title, material, onClose }) => (
  <div
      ...
      .then(data => {
        ...
        setSopItems(data.duties.map((duty: any) => ({
          ...
          completedAt: duty.completedAt
            ...
            : undefined,
          // Carried through explicitly, like every other field here. This mapper
          // builds a fresh object rather than spreading, so anything the server
          // adds is dropped unless it is named -- and an optional field being
          // dropped is silent, which is how the training link rendered nowhere
          // while the API was returning it correctly.
          material: duty.material || null
        })));
      })
          ...
          return (
            ...
            >
              ...
              <div className="flex-1 space-y-1.5">
                ...
                {item.material && (item.material.section || item.material.video) && (
                  <button
                    type="button"
                    onClick={() => setViewingMaterial(item)}
                    className="mt-1 inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full bg-indigo-50 border border-indigo-200 text-indigo-800 text-[11px] font-bold hover:bg-indigo-100 transition cursor-pointer"
                  >
                    {item.material.video
                      ? <PlayCircle className="w-3.5 h-3.5" />
                      : <BookOpen className="w-3.5 h-3.5" />}
                    <span>{item.material.video ? '先看示範（30 秒）' : '先看規範'}</span>
                  </button>
                )}
