  ...
  return (
    <div className="fixed inset-0 z-50 bg-[#5A5A40]/60 backdrop-blur-xs flex items-center justify-center p-4 overflow-y-auto font-sans">
      <div className="bg-white rounded-[32px] max-w-4xl w-full shadow-2xl border border-[#5A5A40]/20 overflow-hidden my-6 animate-in fade-in zoom-in-95 duration-200">
        ...
        <div className="p-6 bg-[#f5f5f0] overflow-x-auto">
          
          <div className="min-w-[700px] flex justify-center">
            {/* Real printable container for html2canvas capture */}
            <div
              ref={certificateRef}
              className="w-[842px] h-[595px] bg-[#FCFAF5] p-10 border-[12px] border-[#5A5A40] rounded-xl shadow-xl relative flex flex-col justify-between text-[#333333] select-none"
              style={{
                boxSizing: 'border-[#5A5A40]'
              }}
            >
              {/* Decorative Double Border */}
              <div className="absolute inset-3 border-2 border-dashed border-[#5A5A40]/30 rounded-lg pointer-events-none"></div>

              {/* Watermark Logo Background */}
              <div className="absolute inset-0 flex items-center justify-center opacity-[0.03] pointer-events-none">
                <div className="text-[160px] font-black font-serif text-[#5A5A40]">🐾 PAW</div>
              </div>

              {/* Certificate Header */}
              <div className="text-center space-y-2 pt-2 relative z-10">
                <div className="flex items-center justify-center space-x-2 text-[#5A5A40] font-bold text-xs uppercase tracking-widest">
                  <ShieldCheck className="w-4 h-4 text-[#5A5A40]" />
                  <span>PawRescue Animal Shelter Volunteer Certificate</span>
                  <ShieldCheck className="w-4 h-4 text-[#5A5A40]" />
                </div>
                <h1 className="text-3xl font-extrabold font-serif text-[#5A5A40] tracking-wider italic">
                  浪浪家園 志工服務榮譽證明書
                </h1>
              ...
              <div className="space-y-6 my-auto text-center px-8 relative z-10">
                <p className="text-base text-slate-700 leading-relaxed font-serif">
                  茲證明志工 <strong className="text-2xl text-[#5A5A40] underline decoration-[#5A5A40]/30 underline-offset-8 px-2">{volunteer.name}</strong> 君（LINE 帳號：@{volunteer.lineId}），
                  熱心投入浪浪家園園區保護流浪動物、洗澡放風、貓舍照護與園區運作之志工服務。
                  服務期間盡心盡力，特頒此證以資感謝與鼓勵！
                </p>

                {/* Key Stats Banner */}
                <div className="bg-[#E6E2D3]/40 border border-[#5A5A40]/20 rounded-2xl p-5 grid grid-cols-3 gap-4 max-w-xl mx-auto shadow-2xs">
                  <div className="text-center">
                    <p className="text-xs text-[#5A5A40] font-bold">累積服務時數</p>
                  ...
                  <div className="text-center border-x border-[#5A5A40]/15">
                    <p className="text-xs text-[#5A5A40] font-bold">完成班次總數</p>
                  ...
                  <div className="text-center">
                    <p className="text-xs text-[#5A5A40] font-bold">志工榮譽階級</p>
        ...
        {/* Action Controls */}
        <div className="bg-[#f5f5f0] p-5 border-t border-[#5A5A40]/10 flex flex-col sm:flex-row justify-between items-center gap-3">
          ...
          <div className="flex items-center space-x-3">
            ...
            <button
              onClick={onClose}
              className="px-4 py-2 bg-white border border-[#5A5A40]/20 text-slate-700 rounded-full font-bold text-xs hover:bg-slate-100 transition cursor-pointer"
            >
              取消
            </button>
            ...
            <button
              ...
              className="bg-[#5A5A40] hover:bg-[#484833] text-white font-extrabold px-6 py-2.5 rounded-full text-xs shadow-md transition flex items-center space-x-2 cursor-pointer disabled:opacity-50"
            >
              {isGenerating ? (
                <>
