...
**上面這些每次 push 都會自動跑一遍**（`.github/workflows/checks.yml`）。在那之前，整條防線的最後一步是「我記得打指令」—— 而忘記不會有任何提示，你只會以為自己跑過了。那正是這個專案最想防的那種沉默失敗，只是它發生在專案自己的流程裡。

CI 會另外建置一次、用一個全新的暫存資料庫把伺服器跑起來，再對它跑 `check:security`，所以 276 項全部涵蓋到。它需要一個 `LINE_LOGIN_CHANNEL_ID` 的佔位值（Channel ID 不是機密）—— 少了它，`/api/auth/line-login-url` 會在授權檢查之前先回 503，那一項就永遠是紅的，而**一個永遠紅的 CI 比沒有 CI 更糟**。
...
- **服務沒有備援，資料有。**這兩件事要分開講。資料庫是三層：本機快照、GCS 異地副本、
  以及每天驗證那份異地副本還原得回來（見上面「備份」一節）。服務本身則是單機單行程 ——
  更新時中斷約一秒，機器故障就是全停。要解得先有第二台，而免費額度是一台。
